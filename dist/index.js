...
exports._operateOnSingleDiff = exports.checkForTypesInDeps = exports.checkForLockfileDiff = exports._renderNPMTable = exports.getNPMMetadataForDep = exports.getPnpmMetadataForDep = exports.findNewDependencies = exports.checkForNewDependencies = exports.checkForRelease = void 0;
...
const checkForNewDependencies = (packagePath, packageDiff, duplicationCache, npmRegistryUrl, npmAuthToken) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const dep of newDependencies) {
        ...
        if ("undefined" === typeof peril) {
            const pnpm = yield (0, exports.getPnpmMetadataForDep)(dep);
            if (pnpm && pnpm.length) {
                cacheEntry.pnpmBody = pnpm;
            }
            else if (dep) {
                warn(`Could not get info from pnpm on ${safeLink(dep)}`);
            }
        }
    }
});
...
const getPnpmMetadataForDep = (dep) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise(resolve => {
        child_process.exec(`pnpm why '${dep}' --json`, (err, output) => {
            if (output) {
                ...
                resolve(`
  <details>
    <summary><code>pnpm why ${printDep(dep)}</code> output</summary>
...
exports.getPnpmMetadataForDep = getPnpmMetadataForDep;
...
function renderDepDuplicationCache(cache) {
    ...
    newDependencies
        ...
        .forEach(({ npmData, pnpmBody, packageJSONPaths }) => markdown(`${_renderNPMTable({ usedInPackageJSONPaths: packageJSONPaths.sort(), npmData })}${pnpmBody
        ? `\n${pnpmBody}`
        : ""}`));
}
// Ensure a lockfile change if deps/devDeps changes, in case
// someone has only used `npm install` instead of `pnpm.
const checkForLockfileDiff = (packagePath, packageDiff) => {
    if (packageDiff.dependencies || packageDiff.devDependencies) {
        const lockfilePath = packagePath.replace(/package\.json$/, "pnpm-lock.yaml");
...
function pnpm(options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        renderDepDuplicationCache(depDuplicationCache);
        console.log(depDuplicationCache)
    });
}
